...
type GetViewDependenciesParams struct {
	Relname string
	Nspname string
}
...
type GetViewDependenciesRow struct {
	...
	DependsOnSchemaName string
	DependsOnKind       interface{}
}

func (q *Queries) GetViewDependencies(ctx context.Context, arg GetViewDependenciesParams) ([]GetViewDependenciesRow, error) {
	rows, err := q.db.QueryContext(ctx, getViewDependencies, arg.Relname, arg.Nspname)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&i.DependsOnName, &i.DependsOnSchemaName, &i.DependsOnKind); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
